...
{
    LOG_INF("Async Received data: %s", data);
}
int main(void)
{
    ...
    const char msg[32] = "Hello to you!";

    ret = rf_recv_async(nrf_dev, &recv_async_cb);
    LOG_INF("Async read ret: %d", ret);
    rf_send(nrf_dev, (uint8_t*)msg, strlen(msg));
    for(;;)
    {
        //while(rf_recv(nrf_dev, (uint8_t*)msg, 32));

        //LOG_INF("Recv Data: %s", msg);
        k_msleep(1000);
    }
